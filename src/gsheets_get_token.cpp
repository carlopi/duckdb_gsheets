...
void base64encode(char *output, const char *input, size_t input_length) {
	...
	for (; input_index < input_length; ++output_index) {
		...
		case 1:
			output[output_index] =
			    get_base64_char(((0x03 & input[input_index]) << 4) | ((0xf0 & input[input_index + 1]) >> 4));
			++input_index;
			break;
		case 2:
			output[output_index] =
			    get_base64_char(((0x0f & input[input_index]) << 2) | ((0xc0 & input[input_index + 1]) >> 6));
			++input_index;
			break;
		case 3:
...
TokenDetails get_token(ClientContext &context, const KeyValueSecret *kv_secret) {
	...
	jwt_claim_set["aud"] =
	    "https://accounts.google.com/o/oauth2/token"; /* intended target of the assertion for an access token */
	...
	if (rsa != NULL) {
		...
		if (RSA_sign(NID_sha256, digest, SHA256_DIGEST_LENGTH, sigret, &siglen, rsa)) {
			if (RSA_verify(NID_sha256, digest, SHA256_DIGEST_LENGTH, sigret, siglen, rsa)) {
				...
				std::string response =
				    perform_https_request(context, "oauth2.googleapis.com", "/token", "", HttpMethod::POST, body,
	...
	throw InvalidInputException(
	    "Conversion from private key to token failed. Check email, key format in JSON file (-----BEGIN PRIVATE "
	    "KEY-----\\n ... -----END PRIVATE KEY-----\\n), and expiration date.");
}
...
std::string get_token_and_cache(ClientContext &context, CatalogTransaction &transaction,
	...
	auto new_token_input = CreateSecretInput {new_secret_kv.GetType(),
	...
	secret_manager.RegisterSecret(transaction, make_uniq<const KeyValueSecret>(new_secret_kv),
	                              OnCreateConflict::REPLACE_ON_CONFLICT, persist_type, storage_mode);
