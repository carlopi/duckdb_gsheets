...
unique_ptr<GlobalFunctionData> GSheetCopyFunction::GSheetWriteInitializeGlobal(ClientContext &context,
	...
	if (options.header) {
		std::string response = call_sheets_api(context, spreadsheet_id, token, encoded_sheet_name, sheet_range,
...
void GSheetCopyFunction::GSheetWriteSink(ExecutionContext &context, FunctionData &bind_data_p,
	...
	std::string response = call_sheets_api(context.client, gstate.spreadsheet_id, gstate.token, encoded_sheet_name,
	                                       sheet_range, HttpMethod::POST, request_body);
