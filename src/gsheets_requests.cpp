...
std::string call_sheets_api(ClientContext &db, const std::string &spreadsheet_id, const std::string &token,
                            const std::string &sheet_name, const std::string &sheet_range, HttpMethod method,
	...
	return perform_https_request(db, host, path, token, method, body);
}

std::string delete_sheet_data(ClientContext &db, const std::string &spreadsheet_id, const std::string &token,
	...
	return perform_https_request(db, host, path, token, HttpMethod::POST, "{}");
}

std::string get_spreadsheet_metadata(ClientContext &db, const std::string &spreadsheet_id, const std::string &token) {
	std::string host = "sheets.googleapis.com";
	std::string path = "/v4/spreadsheets/" + spreadsheet_id + "?&fields=sheets.properties";
	return perform_https_request(db, host, path, token, HttpMethod::GET, "");
}
} // namespace duckdb
